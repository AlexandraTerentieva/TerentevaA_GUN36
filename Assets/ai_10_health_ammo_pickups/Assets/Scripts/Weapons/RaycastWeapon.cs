...
public class RaycastWeapon : MonoBehaviour
{
    ...
    {
        isFiring = true;
        if (accumulatedTime > 0.0f) accumulatedTime = 0.0f;
        recoil.Reset();
    }
    ...
    void RaycastSegment(Vector3 start, Vector3 end, Bullet bullet)
    {
        ...
        if (Physics.Raycast(ray, out hitInfo, distance, layerMask))
        {
            ...
            if (sparksPrefab != null)
                Instantiate(sparksPrefab, hitInfo.point, Quaternion.identity);

            bullet.time = maxLifetime;
            end = hitInfo.point;

            // 👇 ПРОВЕРКА НА УНИЧТОЖАЕМЫЙ ОБЪЕКТ
            var destroyable = hitInfo.collider.GetComponent<DestroyOnDamage>();
            if (destroyable != null)
            {
                destroyable.TakeDamage((int)damage);
            }
    ...
    public bool ShouldReload() => ammoCount == 0 && clipCount > 0;
    public bool IsLowAmmo() => ammoCount == 0 && clipCount == 0;
